...
class LOBRecorder:
    def __init__(self, loop, config):
        self._loop = loop
        self.config = config
        self.client = BClient(self.config['exchange']['api_key'],
                              self.config['exchange']['api_secret'])

    async def run(self):
        symbols = await self.get_symbols()
        logger.info(f'Watching symbols: {symbols}')
        mdcm = MultiplexDepthCacheManager(self.client, self._loop, symbols,
    ...
    async def get_symbols(self):
        ...
        symbols = [s['symbol'] for s in response['symbols'] if
                   s['symbol'].endswith('BTC') and
                   not s['symbol'] in self.config['exchange']['pair_blacklist']]
        return symbols
